...
def perception(data):
    ...
    if swerve_percep[0] or swf_percep[0] or lnf_percep[0] or bilateral_percep[0]:
        ...
        if swf_percep[0]:
            isfullspeed = False
            shift_gear(2) if simulator_mode == True else shift_gear(2)
        if swerve_percep[0]:
            isfullspeed = False
            shift_gear(1) if simulator_mode == True else shift_gear(2)
        if isfullspeed == True:
            shift_gear(8) if simulator_mode == True else shift_gear(3)
    else:
        print("111111111")
        steering_angle += wf_percep[0]
...
def swerve_perception():
    global distance_set
    if distance_set[90] < 2.5:
        if distance_set[90] < 1.5:
            if max(distance_set[0:60]) < max(distance_set[121:180]):
                return (True,-0.6)
            else:
                return (True,0.6) 
        return (True, 0.0)
...
def wall_following_perception(distance_apart,data):
    ...
    steering_angle = 0 # the angle between the two laser rays
    THETA = np.pi / 180 * 30 # target distance from wall
    TARGET_DIS = distance_apart # the distance to project the car forward
    LOOK_AHEAD_DIS = 0.5
    P = 0.25
...
def long_narrow_front_perception():
    ...
    for i in range(10):
        if distance_set[90 - i] < distance_set[91 + i]:
            if distance_set[90 - i] < 10.0:
                steering_angle = -0.15
                break
        else:
            if distance_set[91 + i] < 10.0:
                steering_angle = 0.15
                break
    else:
        return (False,0.0)
